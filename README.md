...
- **启动台主页**: 读取 CS:GO 目录头像 + `rev.ini` 昵称, 一键启动游戏 (优先 newloader.exe 优化版, 缺失时自动就位且不覆盖原件; UAC 提权带前置提示/等待态/取消反馈, 轮询 csgo.exe); 服务器在线状态实时显示 (状态 API 真实查询)
...
- **修复工具**: 一键运行维护动作 (缓存清理 / 注册表修复 / 物品库更新 / 启动优化), 风险分级 (低/中/高) 显示
