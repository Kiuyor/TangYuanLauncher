...
3. **隐藏技术细节**: 面向普通玩家,对用户无价值的内部细节(执行命令、字段描述小字、时间戳、冗余状态文字、**rev.ini 键名标签、代码注释式 kicker 装饰**)主动删除,用户不需要知道这些。(2026-08-30 v2.3.1 三轮: CodeTag/kicker 已按本条删除; mono 字体仅限真代码场景: 启动命令/启动参数/IP:端口/CFG 数值)
4. **主操作突出**: 主操作按钮(启动)始终是页面最大视觉元素,优先放大强调;次要操作弱化。
5. **字体必须随包分发**: 设计字体(HarmonyOS Sans SC + JetBrains Mono,见 tokens.md §字体族)必须随安装包携带 ttf 并自动安装/注册(或打包时嵌入),**禁止依赖目标机器已装字体**——否则陌生用户机器会回退默认字体,出现"一细一粗"渲染缺陷(2026-08 实机教训)。安装脚本见 `inno-setup` 打包配置。**设计字重必须限于随包字重集(HarmonyOS Sans SC 400/500/700)**:v2.3.1 起弃用 800/Black(未随包, 页面标题已改 700), 新增字重须先随包再使用。
6. **动效纪律** (v2.3.1 三轮): 只允许 opacity / offset / scale 三类动画属性; 时长一律 150~250ms + `ease-standard`, 引用 motion 令牌 (tokens.md §7); 每页循环动画 ≤1 个; 禁止布局动画 (宽高过渡); 实机卡顿/异常就地降级为即时切换; HTML 预览与 Flet 动画行为一一对应。
7. **出厂默认头像随包备份**: 修改头像功能的「恢复默认头像」依赖出厂 `avatar.dat`/`avatar1.dat`,必须随安装包备份(存 `assets/`),**禁止依赖目标机器残留的出厂文件**——玩家改过头像后原文件已被替换,无备份则「恢复默认」失效(2026-08-23 新增)。
...
- [ ] 没有编造数据(人数/状态必须真实)?
- [ ] 没有把执行命令/内部细节暴露给用户? 无键名标签/代码注释装饰? mono 只用于真代码场景?
- [ ] 动画引用 motion 令牌? 只动 opacity/offset/scale? 循环动画每页 ≤1?
- [ ] 启动按钮尺寸/样式符合 design-system.md(110px 圆形)?
- [ ] 打包配置包含字体随包(ttf 安装/注册),不依赖目标机器字体? 设计字重均 ≤700(随包集内)?
- [ ] (若有修改头像)出厂 avatar.dat/avatar1.dat 已随包备份到 assets/,「恢复默认」不依赖目标机器残留?
- [ ] HTML 预览 ↔ tokens.md ↔ theme.py 三方令牌一致(含浅色表)? 组件变更已同步 design-system.md 索引?
...
| Flet 组件库 | `flet_app/components/ui.py`(已建) |
| Flet 令牌模块 | `flet_app/theme.py`(已建: COL_*/FONT_*/RADIUS_*/SPACE_*/WIN_*/SHADOW_*/motion) |
| 旧文档 | `design/`(已删除, 勿再引用; 唯一文档体系 = 本 docs/) |
